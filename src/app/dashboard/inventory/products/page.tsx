...
export default async function Page({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <>
      <PageHeader title="Products stock" description="Every product that carries its own stock count." />
      <InventoryTabs active="/dashboard/inventory/products" />
      <StockFilters action="/dashboard/inventory/products" q={q} />
      <StockTable
        rows={rows}
        total={total}
        page={page}
        perPage={perPage}
        basePath="/dashboard/inventory/products"
        query={{ q }}
        showCost={showCost}
        emptyTitle="No products found"
        emptyDescription="Add a product, or clear the search to see everything."
      />
    </>
  );
}
