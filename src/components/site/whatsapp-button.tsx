 ...
 *
 * WhatsApp is how most Bangladeshi shops actually talk to customers, so this
 * sits above the fold on every storefront page. The number comes from
 * Business settings → Social, and the button simply does not render when it
 * is blank — no placeholder, no dead link.
 */
export function WhatsAppButton({ value, shopName }: { value: string; shopName: string }) {
  const href = whatsappLink(value, `Hello ${shopName}, I have a question about`);
  if (!href) return null;
